...
foreach ($my_tweets as $key => $value) {
	...
	{           
		...
		//STATUS
		$status = substr($my_tweets[$key]->text, 0, 200);
		//created
		$created = $my_tweets[$key]->created_at;


		//output custom here
		$out .= ('<span class="tweet_time"><a target="_blank" href="https://twitter.com/'.$twitter_username.'">'.twitter_time($created).'</a></span>
           <span class="tweet_text"> "'.t($status).' "  </span>');
